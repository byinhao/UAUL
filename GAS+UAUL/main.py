...
def init_args():
    ...
    parser.add_argument("--do_eval", default=False, help="Whether to run eval on the dev/test set.")
    parser.add_argument("--do_direct_eval", default=True, action='store_true',
    ...
    # UAUL parameters
    parser.add_argument("--mc_forward_num", default=5, type=int)
...
class T5FineTuner(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.model = Quad_t5_model(args, args.model_name_or_path)
...
def evaluate(data_loader, model, paradigm, task, sents):
    ...
    for batch in tqdm(data_loader):
        ...
        outs = model.model.t5_model.generate(input_ids=batch['source_ids'].to(device),
...
if args.do_direct_eval:
    ...
    scores = evaluate(test_loader, model, args.paradigm, args.task, sents)
    # print(scores)
    ...
    exp_settings = f"GAS+UAUL; Datset={args.dataset}; seed={args.seed}"
