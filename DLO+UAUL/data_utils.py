...
def order_scores_function(quad_list, cur_sent, model, tokenizer, device):
    q = ["[AT]", "[OT]", "[AC]", "[SP]"]
...
def choose_best_order_global(sents, labels, model, tokenizer, device, top_k):
    q = ["[AT]", "[OT]", "[AC]", "[SP]"]
    all_orders = permutations(q)
    all_orders_list = []
    scores = []
    ...
    for i in range(len(sents)):
        ...
        order_scores = order_scores_function(quad_list, cur_sent, model, tokenizer, device)
...
def get_para_asqp_targets(sents, labels, top_k):
    ...
    optim_orders = choose_best_order_global(sents, labels, model, tokenizer, device, top_k)
